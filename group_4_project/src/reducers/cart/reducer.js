  ...
  switch (type) {
    ...
    case "ADD_TO_LIST":
      for(let i = 0 ; i < state.cartList.cartList.length ; i++){
        if(state.cartList.cartList[i].productId == payload.productId && state.cartList.cartList[i].userId == payload.userId){
          return {cartList: {cartList:[ ...state.cartList.cartList]} };
        }
      }
